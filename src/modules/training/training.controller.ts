...
import { $ref, queryStringTrainingReplacement, trainingIdSchema, trainingReplacementIdSchema } from './training.schema'
import {
...
async function trainingRoutes(server: FastifyInstance) {
  ...
  server.post(
    trainingReplacementRoutesPath.register,
    {
      preHandler: [server.authenticate, server.authorizationLimited],
      schema: {
        tags: [tags.trainingReplacement],
